...
public class VampirismCreateInjection extends VampirismAction {
    ...
    protected ActionConditionResult checkHeldItems(LivingEntity user, INonStandPower power) {
        if ((user.getItemInHand(Hand.OFF_HAND).getItem() == Items.SKELETON_SKULL.getItem() && (user.getMainHandItem().getItem() == InitItems.CRACKED_INJECTION.get() || user.getMainHandItem().getItem() == InitItems.EMPTY_INJECTION.get())) ||
                (user.getMainHandItem().getItem() == Items.SKELETON_SKULL.getItem() && user.getOffhandItem().getItem() == InitItems.CRACKED_INJECTION.get() || user.getOffhandItem().getItem() == InitItems.EMPTY_INJECTION.get())) {
        ...
        if (!user.level.isClientSide()) {
            if (user instanceof PlayerEntity) {
                ...
                if (user.getMainHandItem().getItem() == Items.SKELETON_SKULL.getItem() &&
                        (user.getOffhandItem().getItem() == InitItems.CRACKED_INJECTION.get() || user.getOffhandItem().getItem() == InitItems.EMPTY_INJECTION.get())) {
                    stack = user.getMainHandItem();
                    offstack = user.getOffhandItem();
                    hand = Hand.MAIN_HAND;
                } else if (user.getOffhandItem().getItem() == Items.SKELETON_SKULL.getItem() &&
                        (user.getMainHandItem().getItem() == InitItems.CRACKED_INJECTION.get() || user.getMainHandItem().getItem() == InitItems.EMPTY_INJECTION.get())) {
                ...
                if (((hand == Hand.MAIN_HAND && stack.getCount() == 1) || (hand == Hand.OFF_HAND && offstack.getCount() == 1)) && !playerEntity.isCreative()) {
                    playerEntity.setItemInHand(Hand.MAIN_HAND, new ItemStack(InitItems.VAMPIRIC_INJECTION.get()));
